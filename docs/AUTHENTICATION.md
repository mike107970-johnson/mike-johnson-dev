...
Register separate debug, Play App Signing, and upload-certificate fingerprints as applicable. Never commit keystores or certificate private keys. The web BFF and native deep-link handlers are committed; the native callback must be verified on a signed device build after dependencies and the Android SDK are installed.
