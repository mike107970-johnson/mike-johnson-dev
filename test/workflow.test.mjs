...
test('Android workflow installs, builds, synchronizes, restores Gradle, and builds APK',()=>{for(const command of ['npm ci --ignore-scripts','npm run build','npx cap sync android','gradle -p android wrapper --gradle-version 8.11.1','./android/gradlew assembleDebug'])assert.ok(workflow.includes(command),`missing ${command}`)});
