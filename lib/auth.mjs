...
export const clearAuthCookies=()=>['fv_access','fv_refresh','fv_oauth_state','fv_pkce','fv_auth_next'].map(name=>cookie(name,'',{maxAge:0}));
...
export function oauthStart(next='/profile',platform='web'){
  ...
  return {url:url.toString(),cookies:[cookie('fv_oauth_state',state),cookie('fv_pkce',verifier),cookie('fv_auth_next',safeNext(next))],state};
}
...
export function sessionCookies(session){const expires=Math.max(60,Number(session.expires_in||3600));return [cookie('fv_access',session.access_token,{maxAge:expires}),cookie('fv_refresh',session.refresh_token,{maxAge:60*60*24*30})]}
