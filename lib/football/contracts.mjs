...
export function normalizeLineup(row={}) { const player=p=>({id:integer(p.player?.id),name:p.player?.name||null,number:integer(p.player?.number),position:p.player?.pos||null,grid:p.player?.grid||null,image:image(p.player?.photo),captain:Boolean(p.player?.captain)}); return {team:{id:integer(row.team?.id),name:row.team?.name||null,badge:image(row.team?.logo),colors:row.team?.colors||null},formation:row.formation||null,coach:row.coach?{id:integer(row.coach.id),name:row.coach.name||null,image:image(row.coach.photo)}:null,starting:(row.startXI||[]).map(player),substitutes:(row.substitutes||[]).map(player),unavailable:!row.startXI?.length}; }
export function normalizePlayerStatistics(row={}) { return {team:{id:integer(row.team?.id),name:row.team?.name||null,badge:image(row.team?.logo)},players:(row.players||[]).map(entry=>{const s=entry.statistics?.[0]||{};return {id:integer(entry.player?.id),name:entry.player?.name||null,image:image(entry.player?.photo),number:integer(s.games?.number),position:s.games?.position||null,minutes:integer(s.games?.minutes),rating:s.games?.rating==null?null:Number(s.games.rating),captain:Boolean(s.games?.captain),goals:integer(s.goals?.total),assists:integer(s.goals?.assists),shots:integer(s.shots?.total),passes:integer(s.passes?.total),keyPasses:integer(s.passes?.key),tackles:integer(s.tackles?.total),saves:integer(s.goals?.saves),yellowCards:integer(s.cards?.yellow),redCards:integer(s.cards?.red)}}),unavailable:!row.players?.length}; }
export function normalizeStandings(rows=[]) { const groups=rows[0]?.league?.standings||[];return groups.flat().map(row=>({position:integer(row.rank),team:{id:integer(row.team?.id),name:row.team?.name||null,badge:image(row.team?.logo)},played:integer(row.all?.played),wins:integer(row.all?.win),draws:integer(row.all?.draw),losses:integer(row.all?.lose),goalsFor:integer(row.all?.goals?.for),goalsAgainst:integer(row.all?.goals?.against),goalDifference:integer(row.goalsDiff),points:integer(row.points),form:row.form||null,description:row.description||null})); }
