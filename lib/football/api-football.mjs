  ...
  async request(path,params={}){
    ...
    for(let attempt=0;attempt<=this.maxRetries;attempt++){
      ...
      try{const response=await this.fetch(url,{headers:{'x-apisports-key':this.key,'x-rapidapi-host':this.host,'accept':'application/json'},signal:controller.signal});
        ...
        const body=await response.json();if(body.errors&&(Array.isArray(body.errors)?body.errors.length:Object.keys(body.errors).length)){const detail=JSON.stringify(body.errors).toLowerCase(),restricted=/(subscription|plan|access|coverage)/.test(detail);throw new ProviderError(restricted?'This information is not available from the current data provider.':'Provider rejected the request',{status:restricted?403:502,code:restricted?'SUBSCRIPTION_REQUIRED':'PROVIDER_VALIDATION',retryable:false})}
