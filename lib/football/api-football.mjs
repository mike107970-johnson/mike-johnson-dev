import { normalizeEvent, normalizeFixture, normalizeLineup, normalizePlayerStatistics, normalizeStandings, normalizeStatistics } from './contracts.mjs';
...
export class ApiFootballProvider {
  ...
  playerStatistics(id){return this.request('fixtures/players',{fixture:id}).then(rows=>rows.map(normalizePlayerStatistics))}
  ...
  leagues(params={}){return this.request('leagues',params)} standings(league,season){return this.request('standings',{league,season}).then(normalizeStandings)}
