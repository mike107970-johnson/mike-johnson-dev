...
export class FootballService {
  ...
  fixture(id){const existing=this.cache.peek(`fixture:${id}`)?.value,status=existing?.status;const ttl=status?.isFinished?this.ttl.finished:status?.state==='scheduled'?this.ttl.upcoming:this.ttl.live;return this.cached(`fixture:${id}`,ttl,()=>this.provider.fixture(id),{live:Boolean(status?.isLive)})}
